...
const dataPath = path.join(__dirname, '..', 'data', 'selected-leg-exercises.json');

async function run() {
  const exercises = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
  if (!Array.isArray(exercises) || exercises.length !== 50) throw new Error('Expected exactly 50 selected exercises');
